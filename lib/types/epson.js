...
class Epson extends PrinterType {
  ...
  printNVGraphics(keyCode1, keyCode2, settings) {
    this.buffer = null;
    if (keyCode1 > 126 || keyCode1 < 32) throw new Error("printNVGraphics: keyCode1 must be between 32 and 126");
    if (keyCode2 > 126 || keyCode2 < 32) throw new Error("printNVGraphics: keyCode2 must be between 32 and 126");
    this.append(Buffer.from([0x1D, 0x28, 0x4C, 0x06, 0x00, 0x30, 0x45]));
    this.append(Buffer.from([+keyCode1, +keyCode2]))
    if(settings && settings.doubleWidth) {
      this.append(Buffer.from([0x02]))
    } else {
      this.append(Buffer.from([0x01]))
    }
    if(settings && settings.doubleHeight) {
      this.append(Buffer.from([0x02]))
    } else {
      this.append(Buffer.from([0x01]))
    }
